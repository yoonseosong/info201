...
server <- function(input, output) {
  output$map <- renderPlotly({
    ...
    code_name <- c(
      "physicians", "primary_care_physicians", "medical_groups",
    ...
    resource_map <- ggplot(state_shape) +
      geom_polygon(
        mapping = aes(
          x = long, y = lat, group = group, fill = .data[[chosen_resource]],
  ...
  output$pie <- renderPlotly({
    ...
    plot_ly(
      data = state_dat_selected,
      labels = ~RACE,
      values = ~pop, type = "pie"
    ) %>%
      layout(
        title = paste("Racial Demographics in", input$state),
        xaxis = list(
          showgrid = FALSE,
          zeroline = FALSE,
  ...
  output$pop <- renderText({
    ...
    paste("Total population:",
          format(state_dat_selected$pop, big.mark = ",", scientific = F))
  })

  output$scatter <- renderPlotly({
    s_plot <- plot_ly(scatter_data,
      x = ~sum_total_beds, y = ~sum_total_mds, z = ~sum_total_dsch,
      color = ~health_sys_state
    ) %>%
      filter(health_sys_state %in% input$State) %>%
      group_by(health_sys_state) %>%
      add_markers()
    s_plot <- s_plot %>% layout(scene = list(
      xaxis = list(title = "Total Beds"),
